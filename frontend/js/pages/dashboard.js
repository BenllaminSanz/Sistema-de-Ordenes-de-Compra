...
async function cargarReqPendientes() {
  ...
  const tituloEl = document.getElementById('titulo-req');
  const esSolicitante = Auth.getUsuario()?.rol === 'solicitante';

  if (tituloEl) {
    tituloEl.innerHTML = `
      <svg width="16" height="16" fill="none" stroke="currentColor" stroke-width="2"
           viewBox="0 0 24 24"><path d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2"/></svg>
      ${esSolicitante ? 'Mis requerimientos pendientes' : 'Requerimientos pendientes de revisión / para generar OC'}
    `;
  }

  UI.spinner(contenedor);
  try {
    const { datos } = await Api.get('/requerimientos?estado=en_revision,aprobado&limite=8');
    // Mostrar: en revisión + aprobados que aún no tienen OC generada
    const pendientes = datos.filter(r => 
      r.estado === 'en_revision' || 
      (r.estado === 'aprobado' && !r.oc_numero && !r.oc_id)
    );
      ...
      <div class="table-wrap">
        <table>
          <thead><tr>
            ...
            <th>Solicitante</th><th>Estado</th><th>Fecha</th><th></th>
          </tr></thead>
